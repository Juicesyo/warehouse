...
var setting = {
        ...
        , token: 'rUBXHttgsfvDJAoZ' // token可以增加并发次数，用来打码，采集题库奖励.这不是必须申请的,没有token也可以正常使用.token是我们识别用户的一个令牌,脚本会收集你的正确答案,我们会用token来对这些数据进行统计和拦截恶意的提交.另外会用token进行限流操作(ip不可靠),防止恶意的进行cc攻击和误拦截.
